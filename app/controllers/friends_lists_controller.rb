class FriendsListsController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to friends_lists_path, status: :see_other, notice: "Friends list was successfully destroyed." }
  ...
  private
    ...
    def set_friends_list
      @friends_list = FriendsList.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def friends_list_params
      params.expect(friends_list: [ :first_name, :last_name, :contact, :twitter ])
    end
end
